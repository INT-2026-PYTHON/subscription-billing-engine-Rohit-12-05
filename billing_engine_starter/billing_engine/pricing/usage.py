...
class UsageBased(PricingStrategy):
    ...
    def __init__(self, unit_price: Money) -> None:
        if not isinstance(unit_price, Money):
            raise TypeError("UsageBased unit price must be a Money instance.")
        if unit_price.is_negative():
            raise ValueError("UsageBased unit price cannot be negative.")
            
        self.unit_price = unit_price

    def calculate(self, quantity: int) -> Money:
        if quantity < 0:
            raise ValueError("UsageBased quantity cannot be negative.")
